...
import oxrt.frametimes.utils.DynamicJsObject
...
// BEGIN[wind-gradient]
/** @param plugins
  *   Inline plugins for this chart.
  *   - See [[https://www.chartjs.org/docs/latest/configuration/ Chart.js docs]] and
  *     [[https://www.chartjs.org/docs/latest/developers/plugins.html API]]
  */
class ChartConfig(
    @JSName("type")
    var typ: String = null,
...
/** @param label
  *   Caption for this dataset
  *
  * @param data
  *   Array of records. Actual records can be in very different shapes. See Chart.js docs.
  *
  * @param parsing
  *   How to parse the dataset.
  *   - The parsing can be disabled by specifying `parsing: false`.
  *   - If parsing is disabled, data must be sorted and in the formats the associated chart type and scales use
  *     internally.
  *
  * Example data: `[{id: 'Sales', nested: {value: 1500}}, {id: 'Purchases', nested: {value: 500}}]` And its parsing
  * config for bar chart: `js.Dictionary("xAxisKey" -> "id", "yAxisKey" -> "nested.value")`
  */
class ChartDataset(
    var label: String,
    @JSName("type")
...
/** @param parsing
  ...
  *
  * @param scales
  *   Define the scales: scaleId => { type: "logarithmic", position: "right" }
  ...
  *
  * @param plugins
  *   Requires a pluginId => pluginConfig dictionary.
  *   - Pass `false` to disable a specific plugin, or all plugins.
  *   - See [[https://www.chartjs.org/docs/latest/developers/plugins.html Chart.js docs]] and
  *     [[https://www.chartjs.org/docs/latest/api/interfaces/Plugin.html API]]
  */
...
/** @param duration
  *   in milliseconds
  * @param easing
  *   "linear" | "easeInQuad" | "easeOutSine" etc. See
  *   [[https://www.chartjs.org/docs/latest/configuration/animations.html#easing full list]]
  * @param delay
  *   delay before the animation starts, in milliseconds
  * @param loop
  *   if true, animation will loop endlessly
  */
